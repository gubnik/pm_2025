...
\subsection{Скорости точек}
\begin{itemize}
{% for p in points %}
\item Точка ${{ p }}$:
    \begin{align*}
        \dot{x}_{{ p }}(\phi) &= {{ phi_print(ctx['v_x_'+p]) }} \\
                              &= {{ phi_print(subs(ctx['v_x_'+p], diff(phi), symb('omega'))) }}\\
        \dot{y}_{{ p }}(\phi) &= {{ phi_print(ctx['v_y_'+p]) }} \\
                              &= {{ phi_print(subs(ctx['v_y_'+p], diff(phi), symb('omega'))) }}
    \end{align*}
{% endfor %}
\end{itemize}
\begin{itemize}
...
\item Точка ${{ p }}$:
    \begin{align*}
        \dot{x}_{{ p }}(\phi) &= {{ phi_print(ctx['v_x_'+p]) -}} \\
        {% set v_x_first = phi_print(ctx['v_x_'+p]) -%}
        {% set v_x_second = phi_print(subs(ctx['v_x_'+p], diff(phi), symb('omega'))) -%}
        {% if v_x_first != v_x_second -%}
            &= {{ v_x_second -}}\\
        {% endif -%}
        \dot{y}_{{ p }}(\phi) &= {{ phi_print(ctx['v_y_'+p]) -}} \\
        {% set v_y_first = phi_print(ctx['v_y_'+p]) -%}
        {% set v_y_second = phi_print(subs(ctx['v_y_'+p], diff(phi), symb('omega'))) -%}
        {% if v_y_first != v_y_second -%}
            &= {{ v_y_second -}}\\
        {% endif -%}
    \end{align*}
{% endfor -%}

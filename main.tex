...
\item Точка ${{ p }}$:
    \begin{align*}
        \dot{x}_{{ p }}(\phi) &= {{ phi_print(ctx['v_x_'+p]) }} = {{ phi_print(subs(ctx['v_x_'+p], diff(phi), symb('omega'))) }}\\
        \dot{y}_{{ p }}(\phi) &= {{ phi_print(ctx['v_y_'+p]) }} = {{ phi_print(subs(ctx['v_y_'+p], diff(phi), symb('omega'))) }}
    \end{align*}
{% endfor %}
\end{itemize}
